// Script para adicionar produtos reais com imagens de exemplo
...
export const addRealProducts = async () => {
  console.log('🔄 Adicionando produtos reais...')
  
  try {
    // Primeiro, vamos limpar os produtos existentes (opcional)
    console.log('📦 Produtos a serem adicionados:', realProducts.length)
    ...
    console.log('🎉 Todos os produtos reais foram adicionados!')
    return true
  } catch (error) {
...
export const runAddProducts = () => {
  addRealProducts().then(success => {
    if (success) {
      alert('✅ Produtos reais adicionados com sucesso!')
      // Recarregar a página para ver os novos produtos
...
if (typeof window !== 'undefined') {
  (window as any).addRealProducts = runAddProducts
}
